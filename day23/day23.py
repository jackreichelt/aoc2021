...
class Hallway():
  ...
  def nextStep(self, step):
    ...
    if predictedScore > self.maxScore + 1:
      return True
    ...
    for move in validMoves:
      ...
      if newPredictedScore <= self.maxScore + 1:
...
score, state = test.computeStateFromMovelist(testMovelist)
print('Total score', score)
print('Score estimate:', score + test.minRemainingCost(state))
test.printState(state)

test = Hallway(LAYOUT2, 0)
score, state = test.computeStateFromMovelist([])
print('Score estimate:', test.minRemainingCost(state))
if not test.allCellsCorrect(test.initialCells):
  print('somethings fucky')
else:
  print('checking correct works')
